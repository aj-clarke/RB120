...
ANSWER:
Yes, when you change `attr_reader` to `attr_accessor` you are now exposing both
instance variables to the rest of the code. This would allow unintentional
changing of product names via the `product_name` setter method.

Result: On the right track read below
